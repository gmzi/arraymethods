...
function hasAZero(num) {
  let input = num.toFixed();
  let arrayed = [];
  for (let num of input) {
    arrayed.push(num);
  }
  let hasZero = arrayed.some(function (num) {
    return num === '0';
  });
  return hasZero;
}
...
If any of the values in the array are not odd, the function should 
return false. 
...
function hasCertainKey(arr, key) {
  let hasKey = arr.every(function (object) {
...
function hasCertainValue(arr, key, searchValue) {
  let hasValue = arr.every(function (object) {
    return Object.values(object).includes(searchValue);
  });
  return hasValue;
}
